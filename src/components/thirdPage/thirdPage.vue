<template>
  <div class="photo-box">
    ...
    <div class="alertTitle">请确保:舌头在圈定范围内、将舌自然地伸出口外、舌面平展、不逆光</div>
    <div class="BottomBg">
      <mu-flex class="flex-wrapper" align-items="center" justify-content="around">
        ...
        <mu-button color="primary" class="startPhotoBg"  @click="nextStep">进入问诊</mu-button>
      </mu-flex>
    </div>
    <mu-container>
      <mu-dialog  width="360" :open.sync="openSimple">
        ...
        <div class="exampleTitle">舌诊失败,请重新拍照</div>
...
export default {
  ...
  data () {
    return {
      ...
      curIndex: 0,
      imgArr: [
        require('../../assets/tongue-mask.png')
      ],
      imgUrl: '',
      initTouchX: 0,
      initTouchY: 0,
    ...
    createPhoto () {
      if (this.imgUrl) {
        ...
        html2canvas(photoBox, {
          ...
          // that.$store.dispatch('ZW_UPLOAD_TONGUE', file).then(res => {
          //   if (res === true) {
